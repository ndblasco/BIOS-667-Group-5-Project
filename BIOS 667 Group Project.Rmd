...
cdystonia <- cdystonia %>% dplyr::mutate(
    ...
    # adding chg and base so we can potentially use chg as response instead of just TWSTRS
    # and use base as a covariate in the models LR 29NOV2025
    dplyr::mutate(base=twstrs[week==0],
    chg=dplyr::if_else(week != 0, twstrs-base,NA)) %>%
  ungroup()
...
ggplot(data=cdystonia,aes(x=twstrs)) + geom_histogram(binwidth=5,fill="lightblue",color="black")
```
